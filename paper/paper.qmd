...
subtitle: "Democrats Show Strong Support Leading Up to the Election, with Biden's Steady Backing and Harris's Fluctuations Across Regions and Polls."
author: 
  - Mandy He
  - Wendy Yuan
...
We focus on analyzing trends in polling data rather than forecasting the election outcome to gain a deeper understanding of how and why public opinion changes over time. By examining trends, we can see how specific events, regions, and campaign efforts impact voter preferences, providing insights that pure forecasting might miss. Trend analysis also avoids the need for assumptions about future behavior, giving us a clearer picture of the factors that shape current voter support. This approach helps campaigns and analysts recognize consistent patterns in support, guiding more targeted strategies without relying on uncertain predictions.
...
The remainder of the paper is structured as follows: @sec-data details the data and measurement process. @sec-Methodology describes the methodology, including linear and Bayesian models. @sec-Results presents the results, highlighting trends in polling, and @sec-Discussion considers the implications of these results for future research on polling and public opinion.
...
Once this data is collected, it’s cleaned to ensure accuracy by selecting numerical grade that is equal to 3, removing any inconsistencies or missing values. This allows us to analyze and track how public opinion changes over time and in different regions, turning individual responses into a clear and usable dataset.
...
We use the statistical programming language R [@citeR] and the following packages: tidyverse [@tidyverse], dplyr [@dplyr], readr [@readr], ggplot2 [@ggplot2], janitor [@janitor], lubridate [@lubridate], broom [@broom], modelsummary [@modelsummary], rstanarm [@rstanarm], and splines [@splines], knitr [@knitr], kableExtra [@kableExtra].
...
#| message: false
#| label: fig-figure1
#| fig-cap: "Simple linear regression between end date and precentage of respondent votes, showing a positive trend for Democrats with a noticeable dip in the middle."
#| fig_caption: true
#| echo: false
...
base_plot <- ggplot(polling_data, aes(x = end_date, y = pct, color = answer)) +
  ...
  theme_classic() +
  labs(y = "Percentage of respondents", x = "End Date", color = "Candidate") + # Add a label for the legend
...
#| message: false
#| label: fig-figure2
#| fig-cap: "Linear regression applied to multiple subsets of the data. Each panel represents a different pollster, with regression lines indicating a generally consistent positive trend."
#| fig_caption: true
#| echo: false
...
#| message: false
#| label: fig-figure3
#| fig-cap: "Linear regression across various states. Each panel reveals a linear relationship with clearer trends."
#| fig_caption: true
#| echo: false
...
#| message: false
#| label: fig-figure4
#| fig-cap: "Bayesian model using spline fit to show a moderate upward trend, where percentage increases over time."
#| fig_caption: true
#| echo: false
...
#| message: false
#| label: fig-figure5
#| fig-cap: "Bayesian model using spline fit with color-coded points for different states showing a moderate upward trend."
#| fig_caption: true
#| echo: false
...
Furthermore, Harris’s polling fluctuations reflect Keyes’s [@reallifemag2023] idea of "data violence," which suggests that standard polling methods may unintentionally reinforce narrow or inconsistent views across different demographic groups. Keyes’s focus on using diverse data sources could help reduce these biases and offer a more accurate picture of candidate support among various population segments.

## Analyzing Pollster-Specific Trends in Candidate Support

@fig-figure2 uses a linear model to illustrate the polling trends for Biden (blue) and Harris (orange) across different pollsters. Each panel represents the polling data from a specific pollster, showing how candidate support varied over time from 2022 to 2024.
...
YouGov is a widely recognized international polling firm known for its political forecasting, especially through its "poll-of-polls" methodology, which aggregates results from numerous individual surveys[@yougov]. YouGov primarily utilizes online panel polling, offering timely and cost-effective results, especially for U.S. presidential elections. According to Cirone and Spirling’s [@cirone] concept of "methodological awareness" is important here, as understanding how each pollster’s methods affect their results can help create a clearer view of national support trends. Below is a detailed exploration of YouGov's polling methodology [@yougov_methodology].
...
-   Sample Size: Given the budget constraints and the need for statistical reliability, the survey would target a sample size of 5,000 respondents, which allows for meaningful subgroup analysis while keeping costs manageable. This should provide a margin of error around ±1.5% at a 95% confidence level.
...
# Appendix C: Summary Statistics for Predictor Variables {#sec-appc}

```{r}
#| warning: false
#| message: false
#| label: fig-figure6
#| fig-cap: "Summary Statistics for Predictor Variables"
#| fig_caption: true
#| echo: false

# Ensure 'end_date' is in date format
polling_data$end_date <- as.Date(polling_data$end_date, format = "%Y-%m-%d")

# Summary statistics for pollster
pollster_summary <- polling_data %>%
  count(pollster) %>%
  arrange(desc(n))

# Summary statistics for state
state_summary <- polling_data %>%
  count(state) %>%
  arrange(desc(n))

# Summary statistics for end_date
end_date_summary <- polling_data %>%
  summarise(
...
# Appendix D: Multi-Linear Regression Result {#sec-appd}

This is a multiple linear regression that runs pct on end_date, pollster, and state. The regression equation is $pct = \alpha + \beta_1\cdot \text{end\_date}+ \beta_2\cdot \text{pollster}+ \beta_3\cdot \text{state} + \epsilon$.

```{r}
#| warning: false
#| message: false
#| label: fig-figure7
#| fig-cap: "Summary of Multi-Linear Regression: Key Statistics"
#| fig_caption: true
#| echo: false

# Run the linear model
linear <- lm(pct ~ end_date + pollster + state, data = polling_data)

# Extract summary information
sumlinear <- summary(linear)

# Extract R-squared and other statistics
r_squared <- sumlinear$r.squared
adj_r_squared <- sumlinear$adj.r.squared
f_statistic <- sumlinear$fstatistic[1]

# Create a summary statistics data frame
model_summary <- data.frame(
  Statistic = c("R-squared", "Adjusted R-squared", "F-statistic"),
  Value = c(r_squared, adj_r_squared, f_statistic)
)
